...
We can do this by introducing slack variables $\xi_i \geq 0$ for each data point, which allow us to 'slack' the constraints.
...
\text{ such that }
y^{(i)}(\langle \mathbf{w}, \mathbf{x}^{(i)} \rangle - b) \geq 1 - \xi_i \text{ for all } i,
