...
Our final prediction is the class with the highest value (we previously normalized these outputs so that we could interpret them as probabilities):
$$
...
Now, we can apply the kernel trick to compute the inner products of $K(\mathbf{X X^\top})$.
...
We need to compute the $n \times n$ kernel matrix $K(\mathbf{X}, \mathbf{X})$.
